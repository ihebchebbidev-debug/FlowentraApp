-- =====================================================================
-- Comprehensive Missing Tables Migration (idempotent — safe to re-run)
-- Date: 2026-05-30
-- Covers all potentially missing tables across all backend modules
-- Run AFTER 20260530_SchemaFixes_And_MissingTables.sql
-- =====================================================================

-- ─────────────────────────────────────────────────────────────────────
-- SECTION 1: Planning module (snake_case tables)
-- Pattern: id (lowercase), "TenantId" (quoted PascalCase), other cols snake_case
-- ─────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS "IX_user_working_hours_day"      ON user_working_hours (day_of_week);
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 2: Stock module (snake_case, FK → "Articles")
-- ─────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS "IX_stock_transactions_article"    ON stock_transactions ("TenantId", article_id);
CREATE INDEX IF NOT EXISTS "IX_stock_transactions_type"       ON stock_transactions ("TenantId", transaction_type);
CREATE INDEX IF NOT EXISTS "IX_stock_transactions_created_at" ON stock_transactions (created_at);

-- ─────────────────────────────────────────────────────────────────────
-- SECTION 3: Plugin Registry (activated_modules — mixed naming)
-- "TenantId" is PascalCase per explicit HasColumnName in DbContext
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 4: HR Module (hr_* snake_case tables)
-- ─────────────────────────────────────────────────────────────────────
...
-- Performance & Recruitment (review_cycles → goals/reviews; job_openings → applicants → interviews/notes)
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 5: Email Accounts / Sync (UUID PKs, PascalCase columns)
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 6: User Signatures & AI Settings (PascalCase)
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 7: Website Builder (WB_* prefix, PascalCase columns)
-- Dependency order: WB_Sites → WB_Pages → WB_PageVersions
--                   WB_GlobalBlocks → WB_GlobalBlockUsages
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 8: Payments (snake_case tables, VARCHAR(50) string PK)
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 9: Retenue à la Source (RSRecords, TEJExportLogs)
-- ─────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS "IX_TEJExportLogs_TenantId"
    ON "TEJExportLogs" ("TenantId");
...
CREATE INDEX IF NOT EXISTS "IX_SupportTickets_TenantId"
    ON "SupportTickets" ("TenantId");
...
-- Dependency order: Definitions → Triggers + Executions → Logs/Approvals/ProcessedEntities
-- ─────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS "IX_WorkflowTriggers_WorkflowId"
    ON "WorkflowTriggers" ("WorkflowId");
CREATE INDEX IF NOT EXISTS "IX_WorkflowTriggers_EntityType"
    ON "WorkflowTriggers" ("EntityType");
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 12: Sync module
-- sync_changes and sync_operation_receipts use BIGSERIAL (int8)
-- SyncFailureLog / SyncPerformanceLog / TokenRefreshLog are NOT ITenantEntity
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 15: Purchases module (PascalCase)
-- GoodsReceipts and ArticleSuppliers already handled in SchemaFixes migration.
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- SECTION 16: Settings & System tables
-- ModuleScopeSettings: queried on every request; must exist for app to start.
-- AppSettings: mixed column names (Id/"TenantId" PascalCase, others snake_case).
-- ─────────────────────────────────────────────────────────────────────

-- NOT an ITenantEntity — string PK, no TenantId
...
-- Mixed: PascalCase Id/"TenantId", snake_case for other cols
