...
const formatOfferContext = async (id: string): Promise<string> => {
  ...
  const parts = [
    ...
    `- Status: ${offer.status}`,
    `- Total Amount: ${offer.totalAmount} ${offer.currency || 'TND'}`,
...
const formatSaleContext = async (id: string): Promise<string> => {
  ...
  const parts = [
    ...
    `- Total Amount: ${sale.totalAmount} ${sale.currency || 'TND'}`,
...
const formatDealContext = async (id: string): Promise<string> => {
  ...
  const parts = [
    ...
    deal.estimatedValue !== undefined && `- Estimated Value: ${deal.estimatedValue} ${deal.currency || 'TND'}`,
