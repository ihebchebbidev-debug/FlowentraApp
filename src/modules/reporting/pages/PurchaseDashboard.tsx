...
export const PurchaseDashboard = () => {
  ...
  const { data, isLoading } = useReportingPurchase();
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('reporting.purchase.spendBySupplier', 'Spend by Supplier')}</CardTitle>
          </CardHeader>
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.spendBySupplier} layout="vertical">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('reporting.purchase.spendByCategory', 'Spend by Category')}</CardTitle>
          </CardHeader>
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data.spendByCategory} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
        ...
        {/* Receipt Status Donut */}
        <Card>
          <CardHeader>
            <CardTitle>{t('reporting.purchase.receiptStatus', 'Receipt Status')}</CardTitle>
          </CardHeader>
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data.receiptStatus} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                  {data.receiptStatus.map((_, index) => (
                    <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                  ))}
                </Pie>
                <RechartsTooltip />
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t('reporting.purchase.poSpendTrend', 'PO Spend Trend')}</CardTitle>
          </CardHeader>
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data.poSpendTrend}>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('reporting.purchase.poDetails', 'Purchase Order Details')}</CardTitle>
        </CardHeader>
        <CardContent>
