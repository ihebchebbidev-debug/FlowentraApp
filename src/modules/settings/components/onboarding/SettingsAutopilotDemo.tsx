  ...
  return (
    <div className="p-4 md:p-6 relative">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div><h1 id="set-demo-title" className="text-xl font-semibold">Settings</h1><p className="text-[11px] text-muted-foreground">Manage users, roles, and system permissions</p></div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-5">
        ...
        <div className="col-span-4 lg:col-span-3">
          <div id="set-demo-sidebar" className="border border-border rounded-lg bg-card overflow-hidden">
            <div className="px-3 pt-3 pb-1 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70">Personal</div>
            {PERSONAL.map(item => <NavRow key={item.id} item={item} active={state.section === item.id} />)}
            <div className="mx-3 my-2 border-t border-border" />
            <div className="px-3 pb-1 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70">Administration</div>
      ...
      {state.showCreateUser && (
        <div className="absolute inset-0 z-[6] flex items-start justify-center pt-8 bg-background/50">
          <div id="set-demo-create-user" className="w-[460px] bg-card border border-border rounded-xl shadow-2xl p-4">
            ...
            <p className="text-[11px] text-muted-foreground mb-3">Add a new member to your workspace</p>
        ...
        <div className="absolute inset-0 z-[6] flex items-start justify-center pt-6 bg-background/50">
          <div className="w-[680px] bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
            <div className="px-5 pt-4 pb-3 border-b border-border">
              ...
              <p className="text-[11px] text-muted-foreground">Update role details and permissions</p>
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2"><span className="p-1.5 rounded-md bg-primary/10"><Shield className="h-4 w-4 text-primary" /></span><div><p className="text-xs font-semibold">Permissions for Dispatcher</p><p className="text-[10px] text-muted-foreground">Configure what users with this role can access</p></div></div>
                <div className="flex items-center gap-1.5">
                  <div className="h-7 px-2 rounded-md border border-border text-[10px] inline-flex items-center gap-1 text-muted-foreground"><CheckCircle2 className="h-3 w-3" /> Grant All</div>
                  <div className="h-7 px-2 rounded-md border border-border text-[10px] inline-flex items-center gap-1 text-muted-foreground"><X className="h-3 w-3" /> Revoke All</div>
                </div>
              </div>
              ...
              <div className="max-h-[330px] overflow-hidden space-y-4">
                {PERM_CATEGORIES.map((cat, ci) => (
                  <div key={cat.category}>
                    <div className="flex items-center gap-2 mb-2"><span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{cat.category}</span><div className="flex-1 border-t border-border/60" /></div>
                        ...
                        return (
                          <div key={m.label} id={firstModule ? 'set-demo-perm-module' : undefined} className="border border-border rounded-lg px-3 py-2">
                            <div className="flex items-center gap-2.5 mb-2">
                              ...
                              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${granted === 4 ? 'bg-primary text-primary-foreground' : some ? 'bg-muted text-foreground' : 'border border-border text-muted-foreground'}`}>{granted}/4</span>
                            </div>
                            <div className="grid grid-cols-4 gap-1.5">
                              {m.crud.map((on, idx) => (
                                <div key={idx} id={firstModule && idx === 2 ? 'set-demo-perm-crud' : undefined} className={`flex items-center gap-1.5 px-2 py-1 rounded-md border text-[10px] ${on ? 'bg-primary/10 border-primary/30 font-medium' : 'bg-muted/30 border-border text-muted-foreground'} ${state.permStep >= 2 && firstModule && idx === 2 ? 'ring-2 ring-amber-400' : ''}`}>
                    ...
                    <div className="space-y-1.5">
                      {cat.modules.map(m => (
                        <div key={m.label} className="border border-border rounded-lg px-3 py-2 flex items-center gap-2.5">
                          ...
                          <span className="text-xs font-medium flex-1">{m.label}</span>
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${m.granted ? 'bg-muted text-foreground' : 'border border-border text-muted-foreground'}`}>{m.granted}/{m.total}</span>
              ...
              <div className="flex items-center justify-between pt-3 mt-3 border-t border-border">
                {state.permStep >= 2 ? <span className="text-[10px] px-2 py-0.5 rounded-md border border-amber-400/40 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400">Unsaved changes</span> : <span />}
      ...
      {state.showSkills && (
        <div className="absolute inset-0 z-[6] flex items-start justify-center pt-8 bg-background/50">
          <div className="w-[520px] bg-card border border-border rounded-xl shadow-2xl p-4">
            ...
            <p className="text-[11px] text-muted-foreground mb-3">Assign and manage skills required for this role</p>
                ...
                return (
                  <div key={sk.name} className={`rounded-lg border p-2.5 flex items-start gap-2.5 transition-all ${on ? 'border-border bg-card' : 'border-dashed border-border/50 opacity-40'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2"><span className="text-xs font-semibold">{sk.name}</span><span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${sk.tone}`}>{sk.level}</span></div>
                      <p className="text-[10px] text-muted-foreground">{sk.cat}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div id="set-demo-skill-feeds" className="rounded-lg border border-primary/30 bg-primary/5 p-2.5">
              <p className="text-[11px] font-medium mb-1.5 inline-flex items-center gap-1.5"><Activity className="h-3.5 w-3.5 text-primary" /> Feeds the dispatch board</p>
...
function Panel({ icon: Icon, title, desc, children, id }: { icon: any; title: string; desc?: string; children: React.ReactNode; id?: string }) {
  return (
    <div id={id} className="border border-border rounded-lg bg-card">
      <div className="px-4 py-3 border-b border-border"><div className="flex items-center gap-2"><Icon className="h-4 w-4 text-primary" /><span className="text-sm font-semibold">{title}</span></div>{desc && <p className="text-[11px] text-muted-foreground mt-0.5">{desc}</p>}</div>
  ...
  return (
    <Panel icon={User} title="Profile" desc="Update your personal information" id="set-demo-profile">
      <div className="flex items-center gap-4 mb-4">
        ...
        <div className="h-8 px-3 rounded-md border border-border text-[11px] inline-flex items-center gap-1.5 text-muted-foreground"><Upload className="h-3.5 w-3.5" /> Change photo</div>
      </div>
...
function CompanyPanel() {
  return (
    <Panel icon={Building2} title="Company" desc="Your brand across the app and PDF reports">
      <div className="mb-4">
        <label className="block text-[11px] text-muted-foreground mb-2">Company Logo — appears in sidebar, header, login page & PDF reports</label>
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1.5"><div className="h-8 px-3 rounded-md border border-border text-[11px] inline-flex items-center gap-1.5 text-muted-foreground"><Upload className="h-3.5 w-3.5" /> Upload Logo</div><p className="text-[10px] text-muted-foreground">PNG, JPG, SVG — max 5MB</p></div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3 text-xs">
  ...
  return (
    <Panel icon={Shield} title="Security" desc="Change your password">
      <div className="space-y-3 text-xs max-w-md">
        <div>
          <label className="block text-[11px] text-muted-foreground mb-1">Current password</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="block text-[11px] text-muted-foreground mb-1">New password</label><div className="h-9 px-3 rounded-md border border-border text-sm flex items-center justify-between text-muted-foreground">•••••••• <Eye className="h-3.5 w-3.5" /></div></div>
          <div><label className="block text-[11px] text-muted-foreground mb-1">Confirm password</label><div className="h-9 px-3 rounded-md border border-border text-sm flex items-center justify-between text-muted-foreground">•••••••• <Eye className="h-3.5 w-3.5" /></div></div>
        </div>
    ...
    <Panel icon={Users} title="User Management" desc="Manage user accounts and access">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div id="set-demo-users-filter" className="flex items-center gap-2">
          <div className="h-8 w-44 rounded-md border border-border flex items-center gap-2 px-2.5 text-[11px] text-muted-foreground"><Search className="h-3.5 w-3.5" /> Search users…</div>
          <div className={`h-8 px-2.5 rounded-md border text-[11px] inline-flex items-center gap-1.5 ${state.showUserFilters ? 'border-primary text-primary' : 'border-border text-muted-foreground'}`}><Filter className="h-3.5 w-3.5" /> Filters {state.showUserFilters && <span className="h-4 px-1 rounded bg-primary/15 text-primary text-[10px]">3</span>}</div>
        </div>
      ...
      {state.showUserFilters && (
        <div className="flex gap-2 mb-3">
          {['Status: Active', 'Role: All', 'Country: Tunisia'].map(f => <span key={f} className="h-7 px-2.5 rounded-md border border-border text-[11px] inline-flex items-center gap-1.5 text-muted-foreground">{f}<ChevronDown className="h-3 w-3" /></span>)}
        </div>
      )}
      <div id="set-demo-users-table" className="border border-border rounded-lg overflow-hidden">
        <table className="w-full text-xs">
          <thead><tr className="bg-muted/30 border-b border-border/60 text-muted-foreground uppercase tracking-wide text-[10px]">
              ...
              <tr key={u.email} id={u.fresh ? 'set-demo-user-row' : undefined} className={`border-b border-border/40 last:border-0 ${u.admin ? 'bg-muted/20' : ''} ${u.fresh ? 'bg-primary/5 animate-in fade-in' : ''} ${state.highlightUserRow && u.fresh ? 'ring-1 ring-inset ring-primary/40' : ''}`}>
                <td className="px-3 py-2.5">
                  <div className="flex items-center gap-2.5">
                    <span className={`h-7 w-7 rounded-full flex items-center justify-center text-[10px] font-semibold ${u.admin ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground'}`}>{initials(u.name)}</span>
                    <span className="font-medium">{u.name}</span>
                  </div>
                </td>
                <td className="px-3 py-2.5 text-muted-foreground">{u.email}</td>
                <td className="px-3 py-2.5"><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${u.tone} ${u.admin ? 'border border-primary/30' : ''}`}>{u.role}</span></td>
                <td className="px-3 py-2.5"><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${u.active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-destructive/15 text-destructive'}`}>{u.active ? 'Active' : 'Inactive'}</span></td>
...
function RolesPanel() {
  return (
    <Panel icon={Shield} title="Roles & Permissions" desc="Manage roles and permission assignments">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="h-8 w-44 rounded-md border border-border flex items-center gap-2 px-2.5 text-[11px] text-muted-foreground"><Search className="h-3.5 w-3.5" /> Search roles…</div>
          ...
          <tbody>
            {ROLES.map((r) => (
              <tr key={r.name} className="border-b border-border/40 last:border-0">
                ...
                <td className="px-3 py-2.5"><span className="inline-flex items-center rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium">{r.users} users</span></td>
                <td className="px-3 py-2.5"><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${r.active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-destructive/15 text-destructive'}`}>{r.active ? 'Active' : 'Inactive'}</span></td>
    ...
    <div className="space-y-4">
      <Panel icon={Palette} title="Appearance" desc="Customize how the app looks">
        <div id="set-demo-pref-theme" className="mb-4">
          <p className="text-[11px] text-muted-foreground mb-1.5">Theme</p>
          <div className="grid grid-cols-3 gap-2">
            {[['Light', Sun, !dark], ['Dark', Moon, dark], ['System', Monitor, false]].map(([l, Ic, sel]: any) => (
              <div key={l} className={`h-16 rounded-xl border-2 flex flex-col items-center justify-center gap-1 ${sel ? 'border-primary bg-primary/10' : 'border-border'}`}><Ic className={`h-4 w-4 ${sel ? 'text-primary' : ''}`} /><span className={`text-[10px] ${sel ? 'text-primary font-medium' : ''}`}>{l}</span></div>
            ))}
          </div>
        </div>
        <div className="mb-4">
          <p className="text-[11px] text-muted-foreground mb-1.5">Primary color</p>
        ...
        <div className="flex items-center justify-between">
          <div><p className="text-xs font-medium">Language</p><p className="text-[10px] text-muted-foreground">Select your preferred language</p></div>
      ...
      <Panel icon={Layout} title="Layout" desc="Choose your navigation and data views">
        <div className="mb-4">
          <p className="text-[11px] text-muted-foreground mb-1.5">Navigation style</p>
          <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          <p className="text-[11px] text-muted-foreground mb-1.5">Data view</p>
          <div className="grid grid-cols-2 gap-2">
      ...
      <div className="space-y-2">
        {companies.map(c => (
          <div key={c.name} className={`flex items-center justify-between rounded-lg border p-3 ${c.active ? 'border-primary/40 bg-primary/5' : 'border-border'}`}>
            <div className="flex items-center gap-2.5"><span className="h-9 w-9 rounded-lg bg-muted flex items-center justify-center"><Building2 className="h-4 w-4 text-muted-foreground" /></span><div><p className="text-sm font-medium">{c.name}</p><p className="text-[10px] text-muted-foreground">Own team · branding · data</p></div></div>
            <span className={`h-7 px-2.5 rounded-md text-[11px] inline-flex items-center ${c.active ? 'bg-primary/15 text-primary font-medium' : 'border border-border text-muted-foreground'}`}>{c.tag}</span>
          </div>
        ))}
      </div>
    </Panel>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {cards.map(c => { const Ic = c.icon; return (
          <div key={c.title} className="rounded-lg border border-border p-3 flex items-center gap-3">
            ...
            <div className="min-w-0"><p className="text-xs font-medium">{c.title}</p><p className="text-[10px] text-muted-foreground">{c.desc}</p></div>
          </div>
        ); })}
      </div>
    ...
    <div className="fixed inset-0 z-[110] bg-background flex flex-col select-none">
      ...
      <div className="shrink-0 border-t border-border/60 bg-card px-4 py-3">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          {SET_CHAPTERS.map(ch => (<button key={ch.id} onClick={() => jumpChapter(ch.start)} className={`text-[10px] font-medium px-2 py-0.5 rounded-full transition-colors cursor-pointer ${activeChapter.id === ch.id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/70'}`}>{getChapterTitle(demoLang, ch.id, ch.title)}</button>))}
          <span className="ml-auto text-[10px] text-muted-foreground">{Math.min(stepIndex + 1, SET_STEPS.length)} / {SET_STEPS.length}</span>
        </div>
