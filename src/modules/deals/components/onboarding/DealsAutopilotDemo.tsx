    ...
    <div className="flex flex-col">
      <div className="flex items-center justify-between p-4 border-b border-border bg-card/50">
        <div className="flex items-center gap-3">
          ...
          <div><h1 id="dl-demo-title" className="text-xl font-semibold">Deals</h1><p className="text-px-11 text-muted-foreground">Pipeline &amp; opportunities</p></div>
        </div>
      ...
      <div id="dl-demo-forecast" className="p-4 border-b border-border">
        <div className="rounded-xl border border-border/60 bg-card p-4">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mb-3">
            <div><p className="text-px-11 text-muted-foreground font-medium">Weighted pipeline</p><p className="text-lg font-bold">61 200 TND</p></div>
            <div className="h-8 w-px bg-border" />
            <div><p className="text-px-11 text-muted-foreground font-medium">Open value</p><p className="text-lg font-bold">164 500 TND</p></div>
            <div className="h-8 w-px bg-border" />
            <div><p className="text-px-11 text-muted-foreground font-medium">At risk</p><p className="text-lg font-bold text-amber-600">3</p></div>
            <div className="h-8 w-px bg-border" />
            <div><p className="text-px-11 text-muted-foreground font-medium">Avg. age</p><p className="text-lg font-bold">12d</p></div>
          </div>
            ...
            return (
              <div id={col.id === 'negotiation' ? 'dl-demo-col-negotiation' : undefined} key={col.id} className={`flex-shrink-0 w-60 ${spot ? 'ring-2 ring-primary rounded-lg p-1 -m-1' : ''}`}>
                <div className="flex items-center justify-between px-2 py-1.5 mb-2 rounded-md bg-muted/50">
                  <span className="inline-flex items-center gap-1.5 text-sm font-medium"><span className="h-2.5 w-2.5 rounded-full" style={{ background: col.color }} />{col.label}<span className="ml-1 text-px-10 rounded bg-background px-1.5">{items.length}</span></span>
                  <span className="text-px-10 text-muted-foreground">{fmt(total)}</span>
                </div>
                <div className="space-y-2 min-h-[40px]">
                  {items.map(d => (
                    <div key={d.id} className="p-3 rounded-lg border border-border bg-card border-l-2" style={{ borderLeftColor: col.color }}>
                      ...
                      <p className="text-px-10 text-muted-foreground mt-0.5">{d.customer}</p>
                      <div className="flex items-center justify-between mt-1.5"><span className="text-xs font-semibold">{fmt(d.value)} TND</span><span className="text-px-10 text-muted-foreground">{d.prob}%</span></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <tbody>
                {DEMO_DEALS.map(d => (
                  <tr key={d.id} className="border-b border-border/40 last:border-0">
                    ...
                    <td className="px-3 py-2.5"><span className="inline-flex items-center gap-1.5"><span className="h-5 w-5 rounded-full bg-primary/10 text-primary text-px-9 font-bold inline-flex items-center justify-center">{initials(d.customer)}</span>{d.customer}</span></td>
                    <td className="px-3 py-2.5"><span className={`inline-flex rounded-full px-2 py-0.5 text-px-10 font-medium capitalize ${STAGE_BADGE[d.stage]}`}>{d.stage}</span></td>
                    ...
                    <div className="flex items-center justify-between p-3 border border-border rounded-lg bg-muted/30">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-px-10 text-muted-foreground">Company · contact@medina.tn · +216 71 234 567</p>
                        </div>
                      </div>
                      <span className="text-px-10 text-muted-foreground border border-border rounded px-1.5 py-0.5">Change</span>
                    </div>
                  ) : (
              ...
              <div className="space-y-2">
                ...
                {step >= 2 && (
                  <div className="flex items-center justify-between p-2.5 border border-border rounded-lg bg-muted/30">
                    <div className="flex items-center gap-2"><Package className="h-4 w-4 text-primary" /><div><p className="text-xs font-medium">Cold Room A1</p><p className="text-px-10 text-muted-foreground">Carrier · SN CR-2024-A1</p></div></div>
            ...
            <div id="dl-demo-create-items">
              <DemoCard>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="h-7 px-2.5 rounded-md border border-border text-px-11 text-muted-foreground inline-flex items-center gap-1"><Plus className="h-3 w-3" /> Add items</span>
                </div>
                {step >= 3 ? (
                  <div className="space-y-2">
                      ...
                      <div key={r[0]} className="border border-border rounded-lg p-3 flex items-center justify-between">
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-px-9 rounded bg-muted px-1.5 py-0.5 text-muted-foreground capitalize">{r[1]}</span>
                          </div>
                          <p className="text-px-10 text-muted-foreground mt-1">Qty {r[2]} · Unit {r[3]} TND · <span className="inline-flex items-center gap-0.5"><Package className="h-2.5 w-2.5" />{r[4]}</span></p>
                        </div>
                  ...
                  <div className="text-center py-6 text-muted-foreground"><Package className="h-9 w-9 mx-auto mb-2 opacity-40" /><p className="text-xs">No items added yet</p><p className="text-px-10">Click Add items to pick from your catalog</p></div>
                )}
              </DemoCard>
            </div>
          </div>
          ...
          <div className="flex-1 bg-card border border-border/50 rounded-lg shadow-sm p-4">
            <div className="flex items-center justify-between gap-6 flex-wrap">
              <div className="flex items-center gap-3 min-w-0 flex-wrap">
                ...
                <span className={`inline-flex rounded-full px-2 py-0.5 text-px-10 font-medium capitalize ${STAGE_BADGE[D.stage]}`}>{D.stage}</span>
      ...
      <div className="px-4 py-6 lg:px-6 space-y-6">
        <div id="dl-demo-metrics" className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[[DollarSign, 'Value', `${fmt(D.value)} TND`], [Target, 'Probability', `${D.prob}%`], [Calendar, 'Expected close', '15 Jul 2025'], [Send, 'Source', 'Referral']].map(([Ic, l, v]: any, i) => (
            <div key={i} className="rounded-lg border border-border bg-card p-2.5 flex items-center gap-2"><Ic className="h-4 w-4 text-muted-foreground" /><div className="min-w-0"><p className="text-px-10 text-muted-foreground truncate">{l}</p><p className="text-xs font-semibold truncate">{v}</p></div></div>
          ))}
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              <div className="grid md:grid-cols-2 gap-x-8 gap-y-3 text-xs">
                ...
                <div><p className="text-muted-foreground">Stage</p><span className={`inline-flex rounded-full px-2 py-0.5 text-px-10 font-medium capitalize mt-0.5 ${STAGE_BADGE['negotiation']}`}>negotiation</span></div>
            ...
            <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 flex items-center gap-2">
              ...
              <span className="inline-flex rounded-full px-2 py-0.5 text-px-10 font-medium bg-amber-100 text-amber-700">15 Jun 2025</span>
              <span className="text-px-10 text-amber-700 ml-auto">Follow-up overdue</span>
            </div>
                ...
                {[['Cold room panels', '1', '24,500 TND', '24,500 TND'], ['Installation service', '1', '7,000 TND', '7,000 TND']].map(r => (
                  <tr key={r[0]} className="border-b border-border/40 last:border-0">
                    <td className="px-4 py-2.5 font-medium">
                      {r[0]}
                      <span className="ml-2 inline-flex items-center gap-0.5 text-px-10 text-muted-foreground font-normal"><Package className="h-2.5 w-2.5" /> Cold Room A1</span>
                    </td>
          ...
          <div className="w-[440px] bg-card border border-border rounded-xl shadow-2xl p-4">
            ...
            <div className="space-y-2">
              <div id="dl-demo-convert-sale" className={`flex items-start gap-2.5 p-2.5 rounded-lg border ${state.convertSale ? 'border-primary bg-primary/5' : 'border-border'}`}>
                ...
                <ShoppingCart className="h-4 w-4 text-primary mt-0.5" /><div><p className="text-xs font-medium">Convert to Sale</p><p className="text-px-10 text-muted-foreground">Create a sales order with the same customer and line items.</p></div>
              </div>
              <div id="dl-demo-convert-project" className={`flex items-start gap-2.5 p-2.5 rounded-lg border ${state.convertProject ? 'border-primary bg-primary/5' : 'border-border'}`}>
                ...
                <Briefcase className="h-4 w-4 text-primary mt-0.5" /><div><p className="text-xs font-medium">Convert to Project</p><p className="text-px-10 text-muted-foreground">Spin up a project to deliver the work, linked to this deal.</p></div>
              </div>
            </div>
    ...
    <div className="fixed inset-0 z-[110] bg-background flex flex-col select-none">
      ...
      <div className="shrink-0 border-t border-border/60 bg-card px-4 py-3">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          ...
          <span className="ml-auto text-px-10 text-muted-foreground">{Math.min(stepIndex + 1, DL_STEPS.length)} / {DL_STEPS.length}</span>
        </div>
