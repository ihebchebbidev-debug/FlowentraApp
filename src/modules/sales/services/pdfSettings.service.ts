...
export class PdfSettingsService {
  private static readonly MODULE = 'sales' as const;
  private static readonly STORAGE_KEY = 'pdf-settings';
  ...
  static loadSettings(): PdfSettings {
    ...
    try {
      ...
      console.warn('[SalesPdfSettings] Failed to load from localStorage:', error);
    }
  ...
  static saveSettings(settings: PdfSettings): void {
    ...
    pdfSettingsApi.saveSettings(this.MODULE, settings).catch(error => {
      console.error('[SalesPdfSettings] Background save failed:', error);
    });
  }
