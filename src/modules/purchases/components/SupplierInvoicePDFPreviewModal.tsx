...
import { SupplierInvoicePDFDocument } from './SupplierInvoicePDFDocument';
...
export function SupplierInvoicePDFPreviewModal({ isOpen, onClose, invoice, formatCurrency }: SupplierInvoicePDFPreviewModalProps) {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (isMounted) {
          ...
          setPdfSettings({ ...settings, company: { ...settings.company, logo: logoBase64 || '' } } as any);
        }
  ...
  const handleDownload = useCallback(async () => {
    try {
      ...
      a.href = url;
      a.download = `supplier-invoice-${invoice.invoiceNumber || invoice.id}.pdf`;
      a.click();
      ...
      <DialogContent className={`${isMobile ? 'max-w-[95vw] h-[95vh]' : 'max-w-7xl h-[90vh]'} flex flex-col`}>
        <DialogHeader className="flex flex-row items-center justify-between space-y-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className={isMobile ? 'text-lg' : 'text-xl'}>{t('pdf.invoicePreviewTitle', 'Supplier Invoice PDF')}</DialogTitle>
              <p className="text-sm text-muted-foreground">{invoice.invoiceNumber} - {invoice.supplierName}</p>
            </div>
          </div>
        ...
        <div className="flex-1 min-h-0">
          {isLoading ? (
            ...
            <PDFAnnotationViewer key={`view-${pdfKey}`} document={pdfDocElement} fileName={`supplier-invoice-${invoice.invoiceNumber || invoice.id}.pdf`} isSigningMode={false} onSigningModeChange={() => {}} onAnnotationsChange={() => {}} showToolbar={false} />
          )}
        </div>
        {!isMobile && (
          <>
