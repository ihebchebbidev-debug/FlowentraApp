...
import { GitBranch, ShoppingCart, FileText } from "lucide-react";
...
export function ConvertOfferModal({
  ...
  const handleConvert = () => {
    // Offer conversion only creates a Sale. A Service Order can then be
    // created from that Sale via the Sale detail screen.
    onConvert({ convertToSale: true, convertToServiceOrder: false });
    onOpenChange(false);
  };
        ...
        <div className="space-y-6">
          ...
          <div className="bg-muted/50 p-4 rounded-lg">
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              ...
              <li>• {t('conversion.new_sale_order')}</li>
              <li>• {t('conversion.original_offer_remains')}</li>
              {formDocumentsCount > 0 && (
