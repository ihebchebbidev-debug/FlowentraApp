...
export function ItemsTab({ offer, onItemsUpdated }: ItemsTabProps) {
  ...
  const handleSelectItem = async (item: ArticleItem) => {
    ...
    try {
      ...
      // If this offer has been converted to a sale, also add the item to the linked sale
      if (offer.convertedToSaleId) {
        try {
          const saleId = parseInt(offer.convertedToSaleId, 10);
          if (!isNaN(saleId)) {
            await salesApi.addItem(saleId, {
              type: selectedType as 'article' | 'service',
              itemName: item.name,
              itemCode: item.sku || item.articleNumber || item.serviceCode || '',
              description: item.description || undefined,
            ...
            toast.success(t('itemAlsoAddedToSale') || 'Item also added to linked sale');
          }
        } catch (saleErr) {
          console.warn('Failed to sync item to linked sale:', saleErr);
          toast.warning(t('itemAddedButNotSynced') || 'Item added to offer but could not sync to linked sale');
        }
      }
