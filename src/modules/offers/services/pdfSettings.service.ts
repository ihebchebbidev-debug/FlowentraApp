...
export class PdfSettingsService {
  ...
  private static readonly STORAGE_KEY = 'offer-pdf-settings';
  ...
  static loadSettings(): PdfSettings {
    ...
    try {
      ...
      console.warn('[OffersPdfSettings] Failed to load from localStorage:', error);
    }
  ...
  static saveSettings(settings: PdfSettings): void {
    ...
    pdfSettingsApi.saveSettings(this.MODULE, settings).catch(error => {
      console.error('[OffersPdfSettings] Background save failed:', error);
    });
  }
