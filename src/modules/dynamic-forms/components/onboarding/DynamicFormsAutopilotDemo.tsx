  ...
  return (
    <div className="flex flex-col">
      <div className="flex items-center justify-between p-4 border-b border-border bg-card/50">
        <div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-primary/10"><FileText className="h-6 w-6 text-primary" /></div><div><h1 id="df-demo-title" className="text-xl font-semibold">Dynamic Forms</h1><p className="text-px-11 text-muted-foreground">No-code form builder</p></div></div>
            ...
            <tbody>
              {rows.map(f => (
                <tr key={f.id} className="border-b border-border/40 last:border-0">
                  ...
                  <td className="px-4 py-2.5"><span className={`inline-flex items-center rounded-full px-2 py-0.5 text-px-10 font-medium capitalize ${STATUS_CLS[f.status]}`}>{f.status}</span></td>
  ...
  return (
    <div id="df-demo-builder" className="flex h-full min-h-0">
      ...
      <div id="df-demo-palette" className="w-52 shrink-0 border-r border-border bg-card overflow-hidden p-3">
        ...
        <p className="text-px-10 text-muted-foreground mb-3">Drag onto the canvas</p>
        {Object.entries(PALETTE).map(([group, items], gi) => (
          <div key={group} className="mb-3">
            <p className="text-px-9 font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">{group}</p>
            <div className="space-y-1">
              {items.map(([label, Ic]: any, i) => (
                <div key={label} id={gi === 0 && i === 0 ? 'df-demo-field-basic' : gi === 1 && i === 0 ? 'df-demo-field-choice' : gi === 2 && i === 1 ? 'df-demo-field-advanced' : gi === 3 && i === 0 ? 'df-demo-field-layout' : undefined}
                  className="flex items-center gap-2 p-1.5 rounded-md border border-transparent hover:border-border bg-background text-px-11">
      ...
      <div className="flex-1 min-w-0 overflow-hidden p-4 bg-muted/20">
        <div id="df-demo-canvas" className="max-w-xl mx-auto bg-card border border-border rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <div><p className="text-sm font-semibold">Customer Satisfaction Survey</p><p className="text-px-10 text-muted-foreground">{fields.length} fields{state.multiPage ? ' · 2 pages' : ''}</p></div>
            {state.multiPage && <div id="df-demo-multipage" className="flex items-center gap-1.5 rounded-full border border-primary/30 bg-primary/5 px-2 py-1"><FileStack className="h-3 w-3 text-primary" />{[0, 1].map(p => <span key={p} className={`h-1.5 w-7 rounded-full ${p === 0 ? 'bg-primary' : 'bg-muted'}`} />)}<span className="text-px-9 text-primary font-medium">2 pages</span></div>}
          </div>
            ...
            {fields.map((f, i) => (
              <div key={f.label} className={`rounded-md border p-2 ${f.w === 'half' ? 'w-[calc(50%-0.25rem)]' : 'w-full'} ${i === 0 ? 'border-primary ring-1 ring-primary/30' : 'border-border'}`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-px-11 font-medium truncate">{f.label}</span>
                  {f.badge === 'cond' && <span className="ml-auto inline-flex items-center gap-0.5 text-px-8 px-1 rounded bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"><GitBranch className="h-2 w-2" />if</span>}
                  {f.badge === 'dyn' && <span className="ml-auto inline-flex items-center gap-0.5 text-px-8 px-1 rounded bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"><Database className="h-2 w-2" />live</span>}
                  {f.badge === 'cascade' && <span className="ml-auto inline-flex items-center gap-0.5 text-px-8 px-1 rounded bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400"><Link2 className="h-2 w-2" />linked</span>}
                </div>
                {f.kind === 'rating' ? <div className="flex gap-0.5">{[1, 2, 3, 4, 5].map(s => <Star key={s} className={`h-4 w-4 ${s <= 4 ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground'}`} />)}</div>
                  : f.kind === 'signature' ? <div className="h-10 rounded border border-dashed border-border flex items-center justify-center text-px-9 text-muted-foreground"><PenTool className="h-3 w-3 mr-1" /> Sign here</div>
                    : f.kind === 'textarea' ? <div className="h-10 rounded border border-border bg-background" />
                      : f.kind === 'select' ? <div className="h-7 rounded border border-border bg-background flex items-center justify-between px-2 text-px-10 text-muted-foreground">Choose… <ChevronDown className="h-3 w-3" /></div>
      ...
      <div id="df-demo-props" className="w-60 shrink-0 border-l border-border bg-card overflow-hidden flex flex-col">
        <div className="px-3 py-2 border-b border-border"><p className="text-xs font-semibold">Field properties</p><p className="text-px-10 text-muted-foreground truncate">{state.conditionOn ? '“What went wrong?”' : 'Full Name'}</p></div>
        <div className="flex gap-0.5 px-2 pt-2 text-px-10 border-b border-border">
        ...
        <div className="p-3 text-px-11 space-y-2.5 overflow-hidden">
          {state.propsTab === 'basic' && (
            <div id="df-demo-props-basic" className="space-y-2.5">
              <div className="flex gap-1 text-px-9"><span className="px-2 py-0.5 rounded bg-primary text-primary-foreground">EN</span><span className="px-2 py-0.5 rounded border border-border text-muted-foreground">FR</span></div>
              <div><label className="block text-px-10 text-muted-foreground mb-0.5">Label</label><div className="h-7 rounded border border-border bg-background flex items-center px-2">Full Name</div></div>
              <div id="df-demo-props-hint"><label className="block text-px-10 text-muted-foreground mb-0.5">Hint</label><div className="h-7 rounded border border-border bg-background flex items-center px-2 text-muted-foreground">As on your ID</div></div>
              ...
              <div><label className="block text-px-10 text-muted-foreground mb-1">Width</label><div className="flex gap-1">{['Full', 'Half', 'Third'].map((w, i) => <span key={w} className={`px-1.5 py-0.5 rounded text-px-10 ${i === 1 ? 'bg-primary text-primary-foreground' : 'border border-border text-muted-foreground'}`}>{w}</span>)}</div></div>
            </div>
          )}
          {state.propsTab === 'validation' && (
            <div id="df-demo-props-validation" className="space-y-2.5">
              <div className="grid grid-cols-2 gap-2"><div><label className="block text-px-10 text-muted-foreground mb-0.5">Min length</label><div className="h-7 rounded border border-border bg-background flex items-center px-2">2</div></div><div><label className="block text-px-10 text-muted-foreground mb-0.5">Max length</label><div className="h-7 rounded border border-border bg-background flex items-center px-2">60</div></div></div>
              <div><label className="block text-px-10 text-muted-foreground mb-0.5">Pattern</label><div className="h-7 rounded border border-border bg-background flex items-center px-2 text-muted-foreground font-mono text-px-10">^[^@]+@[^@]+$</div></div>
              <div className="rounded-md border border-green-500/30 bg-green-500/5 p-2 text-px-10 text-green-700 dark:text-green-400 inline-flex items-center gap-1.5"><CheckCircle2 className="h-3 w-3" /> Blocks submit until valid</div>
            </div>
          )}
          {state.propsTab === 'options' && (
            <div id="df-demo-props-options" className="space-y-1.5">
              <label className="block text-px-10 text-muted-foreground">Options (EN / FR)</label>
              {['Installation', 'Repair', 'Maintenance', 'Inspection'].map(o => (
                <div key={o} className="flex items-center gap-1.5 h-7 rounded border border-border bg-background px-2"><GripVertical className="h-3 w-3 text-muted-foreground/50" /><span className="flex-1 text-px-10">{o}</span><X className="h-3 w-3 text-muted-foreground" /></div>
              ))}
              <div className="h-7 rounded border border-dashed border-border flex items-center justify-center gap-1 text-px-10 text-muted-foreground"><Plus className="h-3 w-3" /> Add option</div>
            </div>
          )}
          {state.propsTab === 'logic' && (
            <div id="df-demo-logic" className="space-y-2">
              <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-2 space-y-1.5">
                <p className="text-px-10 font-medium inline-flex items-center gap-1"><GitBranch className="h-3 w-3 text-amber-600" /> Conditional visibility</p>
                <div className="text-px-10 text-muted-foreground">Show this field <b className="text-foreground">when</b></div>
                <div className="h-6 rounded border border-border bg-background flex items-center px-2 text-px-10">Rating</div>
                <div className="h-6 rounded border border-border bg-background flex items-center px-2 text-px-10">is less than</div>
                <div className="h-6 rounded border border-border bg-background flex items-center px-2 text-px-10">3 stars</div>
              </div>
              <p className="text-px-9 text-muted-foreground">The form adapts to each respondent.</p>
            </div>
          )}
          {state.propsTab === 'data' && (
            <div id="df-demo-dynamic" className="space-y-2">
              <div className="flex items-center justify-between"><span className="text-px-10">Use live data</span><span className="h-4 w-7 rounded-full bg-primary relative"><span className="absolute top-0.5 left-3.5 h-3 w-3 rounded-full bg-white" /></span></div>
              <div><label className="block text-px-10 text-muted-foreground mb-0.5">Data source</label><div className="h-7 rounded border border-primary bg-primary/5 flex items-center px-2 text-px-10 text-primary inline-flex gap-1"><Database className="h-3 w-3" /> Contacts</div></div>
              {state.cascadeOn && (
                <div id="df-demo-cascade" className="rounded-md border border-indigo-500/30 bg-indigo-500/5 p-2 space-y-1">
                  <p className="text-px-10 font-medium inline-flex items-center gap-1"><Link2 className="h-3 w-3 text-indigo-600" /> Cascading</p>
                  <div className="text-px-9 text-muted-foreground">Filtered by parent field <b className="text-foreground">Customer</b> → shows that customer’s sites only.</div>
                </div>
              )}
            </div>
          )}
        ...
        <div className="absolute inset-0 z-[6] flex items-center justify-center bg-background/50">
          <div id="df-demo-thankyou" className="w-[460px] bg-card border border-border rounded-xl shadow-2xl p-4">
            ...
            <div className="space-y-2 text-xs">
              <div><label className="block text-px-10 text-muted-foreground mb-0.5">Default message</label><div className="rounded-md border border-border p-2 text-px-11">Thanks! Your response has been recorded.</div></div>
              <div className="rounded-md border border-border p-2 space-y-1"><p className="text-px-10 font-medium inline-flex items-center gap-1"><GitBranch className="h-3 w-3 text-primary" /> Conditional rule</p><p className="text-px-10 text-muted-foreground">If rating ≥ 4 → “Glad you loved it! Leave us a Google review →”</p></div>
              <div className="flex items-center justify-between"><span>Redirect after submit</span><div className="flex items-center gap-2"><div className="h-6 px-2 rounded border border-border text-px-10 flex items-center">yoursite.com</div><div className="h-6 w-10 rounded border border-border text-px-10 flex items-center justify-center">3s</div></div></div>
            </div>
          </div>
        </div>
      )}
      ...
      {state.shareOpen && (
        <div className="absolute inset-0 z-[6] flex items-center justify-center bg-background/50">
          <div id="df-demo-share" className="w-[460px] bg-card border border-border rounded-xl shadow-2xl p-4">
            ...
            <div className="flex items-center gap-2"><div className="flex-1 h-8 rounded-md border border-border bg-background px-2 text-px-11 text-muted-foreground flex items-center truncate">flowentra.app/f/csat-2025</div><div className="h-8 px-2.5 rounded-md bg-primary text-primary-foreground text-px-11 inline-flex items-center gap-1"><Copy className="h-3 w-3" /> Copy</div></div>
          </div>
        </div>
      )}
      {/* Publish dialog */}
  ...
  return (
    <div id={id} className="max-w-md mx-auto bg-card border border-border rounded-xl overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-center gap-2"><div className="h-1.5 flex-1 rounded-full bg-muted overflow-hidden"><div className="h-full bg-primary" style={{ width: step === 0 ? '50%' : '100%' }} /></div><span className="text-px-10 text-muted-foreground">Step {step + 1}/2</span></div>
        {step === 0 ? (
          <>
...
function PagePublic({ state }: { state: DFDemoState }) {
  return (
    <div className="p-6 bg-[linear-gradient(180deg,#f1f5f9_0%,#e2e8f0_100%)] dark:bg-[linear-gradient(180deg,#0f172a_0%,#1e293b_100%)] min-h-full">
      <div className="max-w-md mx-auto mb-3 flex items-center gap-2 text-px-11 text-muted-foreground"><Link2 className="h-3.5 w-3.5" /> flowentra.app/f/csat-2025 · no login</div>
  ...
  return (
    <div className="p-4 md:p-6 relative">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary" /><h1 className="text-lg font-semibold">Customer Satisfaction Survey</h1><span className="inline-flex items-center rounded-full bg-primary/10 text-primary px-2 py-0.5 text-px-10 font-medium">248 responses</span></div>
          ...
          <div className="w-[400px] bg-card border border-border rounded-xl shadow-2xl p-4">
            ...
            <div className="space-y-2">
              {[['All responses → Excel', '248 rows'], ['Selected → PDF', 'branded report']].map(o => (
                <div key={o[0]} className="flex items-center justify-between p-2.5 rounded-lg border border-border"><span className="text-xs font-medium">{o[0]}</span><span className="text-px-10 text-muted-foreground">{o[1]}</span></div>
              ))}
            </div>
          </div>
        </div>
          ...
          <div id="df-demo-export-entity" className="w-[460px] bg-card border border-border rounded-xl shadow-2xl p-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[['Contact', User], ['Service Order', ClipboardList], ['Support Ticket', Ticket]].map(([l, Ic]: any) => (
                <div key={l} className="rounded-lg border border-border p-3 text-center hover:border-primary"><Ic className="h-5 w-5 text-primary mx-auto mb-1.5" /><span className="text-px-11 font-medium">{l}</span></div>
              ))}
            </div>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-[110] bg-background flex flex-col select-none">
      ...
      <div className="shrink-0 border-t border-border/60 bg-card px-4 py-3">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          {DF_CHAPTERS.map(ch => (<button key={ch.id} onClick={() => jumpChapter(ch.start)} className={`text-px-10 font-medium px-2 py-0.5 rounded-full transition-colors cursor-pointer ${activeChapter.id === ch.id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/70'}`}>{getChapterTitle(demoLang, ch.id, ch.title)}</button>))}
          <span className="ml-auto text-px-10 text-muted-foreground">{Math.min(stepIndex + 1, DF_STEPS.length)} / {DF_STEPS.length}</span>
        </div>
