 ...
 *
 * Call once from `src/main.tsx` before render. A future settings screen can
 * call `updateTypography(partial)` to mutate live; overrides persist to
 * localStorage and are re-applied on next boot.
 */
import {
...
const EVENT_NAME = 'typography:changed';
...
function saveOverride(patch: TypographyOverride | null): void {
  if (typeof window === 'undefined') return;
  try {
    if (!patch) window.localStorage.removeItem(STORAGE_KEY);
    else window.localStorage.setItem(STORAGE_KEY, JSON.stringify(patch));
  } catch {
...
/** Mutate typography live (settings panel, per-tenant overrides, etc.). */
export function updateTypography(patch: TypographyOverride): TypographyConfig {
  const existing = (loadOverride() ?? {}) as Record<string, unknown>;
  const nextOverride = { ...existing, ...(patch as Record<string, unknown>) } as TypographyOverride;
  saveOverride(nextOverride);
  return applyTypography(nextOverride);
}

/** Clear all runtime overrides, revert to config defaults. */
export function resetTypography(): TypographyConfig {
  saveOverride(null);
  return applyTypography(null);
}

/** Read the currently applied config (post-merge). */
export function getTypography(): TypographyConfig {
  return current;
}

/** Subscribe to live typography changes. */
export function onTypographyChange(
  listener: (config: TypographyConfig) => void,
): () => void {
  if (typeof window === 'undefined') return () => {};
  const handler = (e: Event) => listener((e as CustomEvent<TypographyConfig>).detail);
